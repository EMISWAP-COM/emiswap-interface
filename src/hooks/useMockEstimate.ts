    ...
    switch (type) {
      case 'swap':
        isEnough = JSBI.greaterThan(
          JSBI.BigInt((ethAmount?.toExact() as any) * 10 ** 18),
          JSBI.BigInt(gasPrice * 10 ** 9 * window['env'].REACT_APP_MIN_GAS_FOR_SWAP)
        );
        break;
      case 'pool':
        isEnough = JSBI.greaterThan(
          JSBI.BigInt((ethAmount?.toExact() as any) * 10 ** 18),
          JSBI.BigInt(gasPrice * 10 ** 9 * window['env'].REACT_APP_MIN_GAS_FOR_POOL)
        );
        break;
      case 'invest':
        isEnough = JSBI.greaterThan(
          JSBI.BigInt((ethAmount?.toExact() as any) * 10 ** 18),
          JSBI.BigInt(gasPrice * 10 ** 9 * window['env'].REACT_APP_MIN_GAS_FOR_INVEST)
        );
        break;
    }
  }
